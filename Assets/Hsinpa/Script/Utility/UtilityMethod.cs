...
namespace Utility {
	public class UtilityMethod {
		...
		public static Dictionary<string, T> EditDictionary<T>(Dictionary<string, T> p_dict, string p_id, T p_value)
		{
			if (p_dict.ContainsKey(p_id))
			{
				p_dict[p_id] = p_value;
			}
			else {
				p_dict.Add(p_id, p_value);
			}

			return p_dict;
		}

		public static Dictionary<string, List<T>> EditDictionaryArray<T>(Dictionary<string, List<T>> p_dict, string p_id, T p_value)
		{
			if (p_dict.ContainsKey(p_id))
			{
				p_dict[p_id].Add(p_value);
			}
			else
			{
				p_dict.Add(p_id, new List<T>() { p_value});
			}
